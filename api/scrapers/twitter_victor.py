...
def sns_check(query, num_tweets=10):
    output = []
    # We decode the parameter since we are calling the function from a separate server and parameters info is binary-enconded
    engine_query = '#' + query.decode('utf-8') + ' -filter:retweets'
    for i, tweet in enumerate(sntwitter.TwitterSearchScraper(engine_query).get_items()):
        ...
        output.append({
            'Tweet_No_vic': str(i+1),
            'Date_vic': str(tweet.date),
            'ID_Username_vic': str(tweet.id) + " - " + str(tweet.username),
            'Content_vic': tweet.content,
            'Sentiment_vic': round(sentimiento.polarity, 2)
        })
...
print(sns_check(b'#Fondales OR (#Fondales#Granada) -filter:retweets', 20)) """