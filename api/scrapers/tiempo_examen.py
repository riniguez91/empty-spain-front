...
def scrape_tiempo_examen(location):
    ...
    try:
        ...
        # Parte examen
        """ header = soup.find(class_='m_table_weather_day_header')
        hour_array = []
        for hour in header.findChildren('div', class_='m_table_weather_day_temp_wrapper', recursive=False):
            hour = re.sub('\s{2,}', '', hour.text)
        ...
        for column in table.findChildren('div', recursive=False):
            ...
            # Parte examen
            temp_by_hours = column.find_all(class_='m_table_weather_day_temp_wrapper')
            output.append({
                ...
                'Temp 8:00_new': re.sub('\s+|\n', '',temp_by_hours[0].text),
                'Temp 14:00_new': re.sub('\s+|\n', '',temp_by_hours[1].text),
                'Temp 20:00_new': re.sub('\s+|\n', '',temp_by_hours[2].text)
            })
...
# print(scrape_tiempo_examen(b'Madrid'))